...
import {
  ...
  userProfileLikeFeedQuery,
  userProfileLikeFeedPaginationQuery,
} from "../../../services";
...
export default function UserPageLikes({ userProfileCard }) {
  ...
  useEffect(() => {
    userProfileLikeFeedQuery(id).then((data) => {
      setPosts(data.edges.map((post) => post.node.postByPostId));
  ...
  const fetchMore = async () => {
    cursor &&
      userProfileLikeFeedPaginationQuery(id, cursor).then((data) => {
        setPosts(
          posts.concat(data.edges.map((post) => post.node.postByPostId))
        );
        setHasMore(data.pageInfo.hasNextPage);
        setCursor(data.pageInfo.endCursor);
      });
